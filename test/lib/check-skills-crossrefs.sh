...
#   2. every per-skill agent override is a known slug
#   3. no duplicate skill `skill` name (the identity key for install/remove)
...
data="$("$CHEZMOI_BIN" execute-template --source "$SRC_DIR" \
  '{{ dict "skills" .skills "agents" .agents | toJson }}')"
...
errors="$(printf '%s' "$data" | jq -r --argjson known "$KNOWN_AGENTS" '
  ...
  | [ ( .agents[] | select($ok[.] | not)
      ...
      ( .skills[] as $s
        | ($s.agents // [])[] | select($ok[.] | not)
        | "\($s.skill): agent \(.) is not a known skills-CLI slug" ),
      ( .skills[]
        | select((.repo | test("^[^/[:space:]]+/[^/[:space:]]+$")
                          or test("^(https?://|git@)") or test("\\.git$")) | not)
        | "\(.skill): repo \(.repo) is not owner/repo or a URL" )
    ]
  + ( [ .skills[].skill ] | group_by(.) | map(select(length > 1))
